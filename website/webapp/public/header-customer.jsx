		...
		if(result.length>0){
			for(var i=0;i<result.length;i++){
				...
				if(result[i].componentId=="nav-rpt"){
					...
					return;
				}else if(result[i].componentId=="nav-rpt-old") {
					menus = [
						{
							link:'customer_home.html',
							name:"我的主页"
...
var MenuHead = React.createClass({
  ...
  render:function(){
      ...
      var menuList = menus.map(
          function(menu,index){
              if(menu.name == '成单报表'){
				return(<li className="competeList" key={index}><a className= {menu.clas} href={menu.link}>{menu.name}</a></li>);
			  }
			  return(<li key={index}><a href={menu.link}  className={_this.props.selected==menu.name?menu.clas+" selected":menu.clas}>{menu.name=="目标查询"?"目标搜索":menu.name}</a></li>);
          }
      );
...
var LoginBox = React.createClass({
    ...
    componentDidMount:function(){
		...
		console.log("231312312333333333333333333333333");
...
var newTop = React.createClass({
    ...
    render:function(){
        return (<div className="top">
			<Logo />
			<MenuHead selected={this.props.selected}/>
			<LoginBox />
			<Guide />
		</div>)
    }
