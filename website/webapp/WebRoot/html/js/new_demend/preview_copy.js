  ...
  render:function(){
    ...
    return(
          <table>
              <tr>
                  <td>业务类型:</td>
                  <td><span className="business-types">{isNull(this.props.data.servicetype)?"":this.props.data.servicetype}</span>
                      <i className="bubble-hover">?</i>
                      <div className="bubble">通过致电目标群体，以标准话术挖掘意向客户</div>
                  </td>
              </tr>
              <tr>
                  <td style={{"vertical-align": "text-top","line-height":"30px"}}>行业：</td>
                  <td className="industry">
                      {industry.map(function(result,index){
                            if(result==data.industry){
                              return (<div className="btn selected">{result}</div>)
                            }else{
                              return (<div className="btn">{result}</div>)
                            }
                        })
                      }
                  </td>
              ...
              <tr className="hide">
                  <td>结算模式</td>
                  <td>
                      <select className="ui-select1 settlement-pattern" name="drop" required>
                      <option ></option>
                      {settlement_pattern.map(function(result,index){
                          if(result==data.paymentstandard){
                            return <option selected>{result}</option>
                          }else{
                      ...
                      <i className="bubble-hover">?</i>
                      <div className="bubble" style={{"width":" 273px"}}>
                          <p><span className="red">查重后抽检：</span>与甲方数据查重后进行录音的质量检验</p>
                          <p><span  className="red">回访电话接通数：</span>甲方回访后的接通数量，剔除无效与未接通数（>4次）</p>
                      </div>
                  </td>
              </tr>
          </table>
    ...
    return(
          <table>
              <tr>
                  ...
                  <td className="zdy">自定义：</td>
                  <td className="zdy"><input type="text" placeholder="请输入自定义人群（12个字以内）"  defaultValue={data.targethumen}/></td>
              </tr>
              <tr>
                  <td>对象年龄：</td>
                  <td className="age"><input type="number" required defaultValue={data.beginage} min="0" max="120" className="startAge"/> - <input type="number" required  min="0" max="120" defaultValue={data.endage} className="endAge"/><span style={{"margin-left":"12px","color":"#999"}}>周岁</span>
  ...
  render:function(){
      return(
                <table>
                    <tr>
                        <td>需求名称：</td>
                        <td><input type="text" className="demand-name" maxLength="12" required defaultValue={this.props.data.demandname}/><span className="info">为您的需求命名，限制 12 个字</span>
                        <p className="error">请填写需求名称</p></td>
                    </tr>
                    <tr>
                        <td style={{"vertical-align":"top","line-height":"30px"}}>需求描述：</td>
                        <td><textarea maxLength="150" className="descriptionDemand" required onInput={this.handleInput.bind(this)}>{this.props.data.demanddescription}</textarea><p><span className="length">还可以输入 <span className="size2">150</span> 个字</span></p>
                        <p className="error">请填写需求描述</p></td>
                    </tr>
                    <tr>
                        <td>产品名称：</td>
                        <td>
                            <input type="text" maxLength="12" defaultValue={this.props.data.productname} className="pro-name" name="proname" autoComplete="on"/><span className="info">您的产品或服务名，限制 12 个字</span>
                    ...
                    <tr>
                        ...
                        <td>
                            <textarea maxLength="150" className="introductionPro" required defaultValue={this.props.data.demand} placeholder="产品或服务介绍，可上传附件，可填入网站链接" onInput={this.handleInput.bind(this)}></textarea>
                    ...
                    <tr>
                        ...
                        <td><input type="text" className="form_date startDate" required autoComplete="off" onChange={this.handleInput.bind(this)} defaultValue={this.props.data.begintime}/><i className="date_icon"></i><span>结束日期:</span><input type="text" autoComplete="off" className="form_date endDate" required onChange={this.handleInput.bind(this)} defaultValue={this.props.data.endtime} /><i className="date_icon"></i>
                        <p className="error" style={{"left":"0","top":"33px"}}></p></td>
                    </tr>
                    <tr>
                        <td style={{"vertical-align":"top","line-height":"30px"}}>项目负责人：</td>
                        <td>
                          <p style={{"marginBottom":"20px"}}>是否需要其他人负责此需求：<label><input type="radio" name="isFZ"  defaultChecked={!(isNull(this.props.data.pleader)&&isNull(this.props.data.pphone))}/>是</label><label><input type="radio" name="isFZ"  defaultChecked={isNull(this.props.data.pleader)&&isNull(this.props.data.pphone)}/>否</label></p>
                          <p className="radio_fz" style={isNull(this.props.data.pleader)&&isNull(this.props.data.pphone)?{"display":"none"}:{"display":"block"}}><span>负责人姓名：</span><input type="text" style={{"width":"162px"}} defaultValue={this.props.data.pleader} className="pleader" /><span style={{"marginLeft":"84px"}}>负责人电话：</span><input type="text" style={{"width":"162px"}} defaultValue={this.props.data.pphone} className="pphone" /></p>
    ...
    return (
      <table>
          <tr>
              ...
              <td width="470px"><label><input type="radio" name="isXS" />是</label><label><input type="radio" name="isXS" defaultChecked />否</label>
...
var AreaList = React.createClass({
  componentDidMount:function(){
      $(this.getDOMNode()).find("div.close").click("click",function(){
          ...
          if($(".form_col4 .main div.areaList").length>1){
              ...
              React.render(<JinE data={areaList()}/>,$("#new_demand div.demend_right .right").eq(1)[0]);
          }else{
            alert("请至少保留一个区域");
          }
      });
        $(this.getDOMNode()).find("input.size").on("input",function(){
      ...
      $(this.getDOMNode()).find("input.money").on("input",function(){
        var total=$(this).parent().parent().find("span.green span.size");
  ...
  render:function(){
    if(isNull(this.props.data)){
         return (
             <div>
                 <div className="input"><label>目标区域:</label><input type="text" required   className="select area" placeholder="请选择区域" /><i className="select_icon"></i>
                 <p className="error">请选择区域</p></div>
                 <div className="input"><label>发布数量:</label><input type="text" required className="number size"/><i>条</i>
                 <p className="error">请填写发布数量</p></div>
                 <div className="input"><label>单价:</label><input type="text" required  className="number money"  /><i>元</i>
                 <p className="error">请填写单价</p></div>
                 <span className="green">总价:<span className="size"> 0 </span>元</span>
                 <div className="close"></div>
                 <div className="input isNote"><label>是否备注:</label><label><input type="radio" name={"note"+this.props.length} /><span>是</span></label><label><input type="radio" name={"note"+this.props.length} defaultChecked /><span>否</span></label></div>
                 <div className="input" style={{"display":"none"}}><label>备注:</label><input type="text" placeholder="填写备注信息" maxLength="50"   /><span className="gray">最多可输入 50 个字</span></div>
             </div>
         )
    }else{
         return (
             <div>
                 <div className="input"><label>目标区域:</label><input type="text" required className="select area" placeholder="请选择区域" defaultValue={this.props.data.targecity} title={this.props.data.targecity}/><i className="select_icon"></i>
                 <p className="error">请选择区域</p></div>
                 <div className="input"><label>发布数量:</label><input type="text" className="number size" defaultValue={this.props.data.releasenum}/><i>条</i>
                 <p className="error">请填写发布数量</p></div>
                 <div className="input"><label>单价:</label><input type="text" className="number money" defaultValue={this.props.data.orderprice} /><i>元</i>
                 <p className="error">请填写单价</p></div>
                 <span className="green">总价:<span className="size">{isNull(this.props.data.orderpricetol)?"0":this.props.data.orderpricetol} </span>元</span>
                 <div className="close"></div>
                 <div className="input isNote"><label>是否备注:</label><label><input type="radio" name={"note"+this.props.length} /><span>是</span></label><label><input type="radio" name={"note"+this.props.length} defaultChecked /><span>否</span></label></div>
                 <div className="input" style={isNull(this.props.data.subdescription)?{"display":"none"}:{"display":"inline-block"}}><label>备注:</label><input type="text" placeholder="填写备注信息" maxLength="50"  defaultValue={this.props.data.subdescription} /><span className="gray">最多可输入 50 个字</span></div>
             </div>
         )}
 },
});
var Fbfb = React.createClass({
  componentDidMount:function(){
      ...
      $(".main div[class^=areaList]:not(.areaList_add) .isNote input[name^=note]").live("change",function(){
          ...
          $("input.area").on("click",function(){
            var index=$(this).parent().parent().parent().index();
              React.render(<Area area={area} province={province} hotCity={hotCity} index={index}/>,$("#new_demand div.demend_left div.area")[0]);
      ...
      $(".areaList_add button").click(function(){
        ...
        if(length_add<=5){
          ...
          React.render(<AreaList length={i++}/>,$(main).find("div.areaList").eq(length_add-1)[0]);
  ...
  render:function(){
    var areaCityList;
    if(isNull(this.props.data.areaCityList)){
      return (
              <div>
                  <div className="areaList_add"><button type="button"><i className="add"></i><span>添加区域</span></button><span>还可以添加<span className="length" > {isNull(this.props.data.areaCityList)?5:5-this.props.data.areaCityList.length} </span>个区域</span></div>
              </div>
      )
    }
       return (
               <div>
               {this.props.data.areaCityList.map(function(result,index){
                      return (
                            ...
                            <div>
                                <table className="cost">
                                    ...
                                    {dom}
                                        ...
                                        <tr>
                                            <td colSpan="4"><span className="money">¥<span className="size">{total}</span></span></td>
                                        </tr>
                                    </tbody>
                                </table>
                                <button type="button" className="submit btn progress_speed_show">提交审核</button><button type="button" className="drafts btn btn_tj2">加入草稿箱</button>
                            </div>
                        </div>
        </div>
        )
...
function getUrlGetDetailHfp(getDetailHfp){
  setTimeout(function(){
    React.render(<Yewu data={getDetailHfp}/>,$("#new_demand div.demend_left .form_col1>.main")[0]);
    React.render(<Mbkh data={getDetailHfp}/>,$("#new_demand div.demend_left .form_col2>.main")[0]);
    React.render(<Jbxx data={getDetailHfp}/>,$("#new_demand div.demend_left .form_col3>.main")[0]);
    React.render(<Fbfb data={getDetailHfp}/>,$("#new_demand div.demend_left .form_col4>.main")[0]);
    React.render(<Fj data={getDetailHfp}/>,$("#new_demand div.demend_left .form_col5>.main")[0]);
    React.render(<Preview data={getDetailHfp}/>,$("#new_demand div.demend_right .right").eq(0)[0]);
    React.render(<Progress_speed data={getDetailHfp}/>,$(".progress_speed")[0]);
    React.render(<JinE data={areaList()}/>,$("#new_demand div.demend_right .right").eq(1)[0]);
