...
class Main extends React.Component{
	...
	componentDidMount(){
     /*拿到并跟新数据*/
		this.getAllPageData({currentPage:1});  //获取第一页的数据
		/*获取当前页*/
	...
	getAllData(callBack,obj){
		...
		if(this.props.state){
			...
			//par订单状态： 1全部 2审核 3进行 4结算 5已完成 6异常需求
			obj.par = $(".search-box li.selected").attr("data-state") || 0;	//获取订单状态类型
		}
...
//TODO 如果par==6 为异常需求
        ...
        $.when($.ajax({
			url:url,
			data:{
				par:obj.par,						//获取订单状态类型
				currentPage:obj.currentPage,		//当前所在页数
				jfuid:jfuid,						//用户id
				begincreatetime:obj.startDate,		//开始时间
				endcreatetime:obj.endDate,			//结束时间
				demandname:obj.ycdemandname,		//需求名称
            ...
            console.log(data.hfpList,66666);
	...
	createMainHtml(){
		...
		return this.state.Alldata.map(function (name,index){
			...
			var pause;
			var changeschedule;
			if(this.props.state=='6'){//异常需求
                pause =  this.isReview(name.fdstate,name.pause);
			}else{
				var  isReview = this.isReview(name.fdstate);//转换状态值，非异常 需求
			}
			if(pause){
				changeschedule = this.changeschedule({pause:pause});//返回审核状态
			}else{
				changeschedule = this.changeschedule({checckSchedule:checckSchedule,releasenum:name.releasenum});//返回cavans图形
			}
			var payPattern = this.payPattern(name.paymentstandard);
			var servicetype = this.servicetype({servicetype:name.servicetype,industry:name.industry,twolevindustry:name.twolevindustry});//根据类型不同，返回不同的链接，可跳转到对应的详情页
			return (<div className="reslut_box" key={index}>
						<div className="title">
							<h3 title={name.demandname}>{name.demandname}</h3>
							<ul>
								<li className={"bg_green "+(name.twolevindustry?'':'hide')}>{name.twolevindustry}</li>  {/*name.twolevindustry 线上或线下 如果没值隐藏*/}
								<li className={"bg_blue1 "+(name.industry?'':'hide')}>{name.industry}</li>{/*发包类型 eg:教育培训..*/}
								<li className="cash"><span>￥</span><span className="font24">{name.demandpricetol.toFixed(2)}</span></li>   {/*金额 .toFixed四舍五入取一定的位数*/}
							</ul>
						</div>
...
class DraftRequire extends React.Component	{
	...
	createDraftHtml(){
		...
		return this.state.DraftData.map(function (name,index){
			var industryservice = this.industryservice({servicetype:name.servicetype,industry:name.industry});//数据类型
