			...
			<table className="messsage-table">
				<tbody>
					...
					<tr>
							...
							<td><textarea maxLength="150" placeholder="输入需求的备注的信息，如无特殊说明，可以不填" className="descriptionDemand" ></textarea><p className="messsage-area"><span className="length">还可以输入 <span className="size2">{150-this.state.descriptionDemandLength}</span> 个字</span></p></td>
					</tr>
					<tr>
...
class Preview extends React.Component{
	...
	save(){
			...
			//var url = "http://192.168.2.33:8091/1_5/addOrUpdateCustomerDemand";
			//debugger;
				$.ajax({
						type:'POST',
	...
	submit(){
		...
		this.rootEl.find("button.btn_tj").click(function(){
			...
			var data={
					...
					"releasenum":releasenum,
					"labelList":labelActive,//收费标签
					"customlabel":customlabel.length!=0?JSON.stringify(customlabel):""
					//自定义标签
			};
...
if(newDandData){
	ReactDOM.render(<Yewu industry={JSON.parse(newDandData).industry} name="1"/>,React.rootEl.find(".form_col1>.main")[0]);
}else{
	ReactDOM.render(<Yewu name="1"/>,React.rootEl.find(".form_col1>.main")[0]);
}

if(newDandData){
	ReactDOM.render(<Mbkh name='1' taskPeople = {JSON.parse(newDandData).taskPeople} />,React.rootEl.find(".form_col2>.main")[0]);
}else{
...
if(newDandData){
	ReactDOM.render(<AreaAndRequirement  city={JSON.parse(newDandData).city} name='1' />,React.rootEl.find(".form_col3>.main")[0]);
...
if(newDandData){
	ReactDOM.render(<SaleTemplate saleTem = {JSON.parse(newDandData).saleTem } name='1' />,React.rootEl.find(".form_col4>.main").eq(1)[0]);
}else{
...
/*描述*/
if(newDandData){
	ReactDOM.render(<EssentialInformation discrible = {JSON.parse(newDandData).discrible } name='1' />,React.rootEl.find(".form_col5>.main")[0]);
}else{
...
ReactDOM.render(<Progress name='1' />,$(".demend_left .progress_speed")[0],function(){console.timeEnd("test");});
