...
class Peoples extends React.Component{
	...
	createBasicHtml(){
		if(this.state.basicData){
			...
			return basicData.map(function (data,index){				
				return (<li key = {index} data-price = {data.labelPrice} >{data.labelName}</li>);
			});
		}
	}
	createChargeHtml(){
	...
	sendMessage(){
		...
		$('.send').click(function (){
			...
			//	location.href =  domain + '/html/new_demand.html';
			//}else{
			//	$(".cover").show();
			//}
		});
		...
		return (
				<div className="education">
					...
					<div className="taskPeople">
						...
						<p className="error first task-people"  style={{"display":"none","marginLeft":"92px","marginTop":"70px"}}>结束年龄必须大于开始年龄！</p>
					</div>
