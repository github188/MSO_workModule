import addDemand from "./postDate.js";
var createTime;
var file1;
var file2;
var file3;
...
function getUrlGetDetailHfp(getDetailHfp) {
    sessionStorage.setItem("cgid",getDetailHfp.demand.pid);
    setTimeout(function () {
        ReactDOM.render(<DateFilter data={getDetailHfp.demand}/>, $(".clo1")[0]);
        ReactDOM.render(<BasicInformation data={getDetailHfp.demand}/>, $(".clo2")[0]);
        ReactDOM.render(<Annex data={getDetailHfp.demand}/>, $(".clo3")[0]);
...
var url = `${domain137}/quality/demanddetail/${location.hash.slice(1)}`;
$.ajax({
    type: "GET",
    url: url,
    dataType: "json",
...
// } else {
//
//     console.log(JSON.parse(sessionStorage.getItem(urlGetDetailHfp)));
...
class DateFilter extends React.Component {
    ...
    handleInput() {
        $(".right").find("span.num").html(Number($("input.num").val()) || 0)
        $(".right").find("span.money").html(Number($("input.money").val()) || 0)
        $(".right").find("span.total").html((Number($("input.num").val()) || 0) * (Number($("input.money").val()) || 0))
        ...
        return (  <div className="block-box">
            ...
            <div className="content-right">
                <ul>
                    <li>
                        <label>业务类型：</label>
                        <span className="types-of">{this.state.servicetype[data.servicetype]}</span><i><span
                        className="help">?</span><span
                        className="bubble">针对企业自有数据和需求，进行有效触达</span></i>
                    </li>
                    <li>
                        <label>清洗渠道：</label>
                        <span>电话</span>
                    ...
                    <li>
                        <label>数据条目：</label>
                        <input autoComplete="off" type="text" className="num" onInput={this.handleInput.bind(this)}
                               defaultValue={data.releasenum}/>
                    ...
                    <li>
                        <label>单价：</label>
                        <input autoComplete="off" type="text" className="money"
                               onInput={this.handleInput.bind(this)} defaultValue={data.pprice}/>
                        <span style={{"color": "#777777"}}>元</span>
                        <p className="error">请填写单价</p>
                    </li>
                    <li className="hide">
                        <label>结算模式：</label>
                        <span>数据条目</span><i><span className="help">?</span><span
                        className="bubble">按实际提交数据条目数乘以单价进行结算</span></i>
                    </li>
                </ul>
            </div>
        </div>)
...
class BasicInformation extends React.Component {
    componentDidMount() {
        ...
        $("input.startDate").blur(function () {
            setTimeout(function () {
                if($("input.startDate").val() == "" || $("input.endDate").val() == ""){
                    $("input.startDate").parent().find("p.error").show();
                }else{
                    $("input.startDate").parent().find("p.error").hide();
                }

                if($("input.endDate").val()==""){
                    $("input.endDate").addClass("error").parent().find("p.error").text("请填写结束日期")
                }else{
                    $("input.endDate").removeClass("error")
                }

                if($("input.startDate").val()==""){
                    $("input.startDate").addClass("error").parent().find("p.error").text("请填写开始日期")
                }else{
                    $("input.startDate").removeClass("error")
                }
        ...
        $("input.endDate").blur(function () {
            setTimeout(function () {
                if($("input.startDate").val() == "" || $("input.endDate").val() == ""){
                    $("input.endDate").parent().find("p.error").show();
                }else{
                    $("input.endDate").parent().find("p.error").hide();
                }
            ...
            <div className="content-right">
                <div className="main">
                    <table>
                        ...
                        <tr>
                            <td>需求名称：</td>
                            <td><input autoComplete="off" type="text" defaultValue={data.demandname}
                                       className="demand-name" maxLength="12"/><span
                                className="info">为您的需求命名，限制 12 个字</span>
                                <p className="error">请填写需求名称</p></td>
                        </tr>
                        <tr>
                            <td style={{"verticalAlign": "top"}}>需求描述：</td>
                            <td><textarea maxLength="150" defaultValue={data.demanddescription}
                                          className="descriptionDemand"
                                          onInput={this.handleInput.bind(this)}></textarea><p><span
                                className="length">还可以输入 <span className="size2">150</span> 个字</span></p>
                        ...
                        <tr>
                            ...
                            <td>
                                <input autoComplete="off" type="text" defaultValue={""}
                                       className="form_date startDate" readOnly
                                       onChange={this.handleInput.bind(this)}/><i
                                className="date_icon"></i><span>结束日期:</span><input
                                type="text" defaultValue={""} className="form_date endDate" readOnly
                                onChange={this.handleInput.bind(this)}/><i className="date_icon"></i>
                                <p className="error" style={{position: "relative", left: 0}}></p>
                            </td>
                        </tr>
                        <tr>
                            <td style={{"verticalAlign": "top"}}>项目负责人：</td>
                            <td>
                                <p style={{"marginBottom": "20px"}}>是否需要其他人负责此需求：<label>
                                    <input type="radio" name="isFZ"
                                           defaultChecked={(data.pleader || data.pphone) ? true : false}/>是</label>
                                    <label><input type="radio" name="isFZ"
                                                  defaultChecked={(data.pleader || data.pphone) ? false : true}/>否</label>
                                </p>
                                <p className="radio_fz"
                                   style={{display: (data.pleader || data.pphone) ? "block" : "none"}}>
                                    <span>负责人姓名：</span>
                                    <input autoComplete="off" type="text" defaultValue={data.pleader}
                                           style={{"width": "162px"}} className="pleader"/>
                                    <span style={{"marginLeft": "84px"}}>负责人电话：</span>
                                    <input autoComplete="off" type="text" defaultValue={data.pphone}
                                           className="pphone" style={{"width": "162px"}}/>
                                </p>
 ...
 * 右侧需求预览
 */
class PreviewRight extends React.Component {
    componentDidMount() {
