		...
		return (
		    <table>
                <tr>
                    ...
                    <td className="zdy"><input autoComplete="off" type="text" placeholder="请输入自定义人群（12个字以内）" className="mbrq"/></td>
                </tr>
                <tr>
                    <td>对象年龄：</td>
                    <td className="age"><input autoComplete="off" type="number" className="startAge" min="0" max="120" required/> - <input autoComplete="off" type="number" className="endAge" min="0" max="120" required/><span style={{"margin-left":"12px","color":"#999"}} >周岁</span>
	...
	render(){
		return (
			<table>
				<tr>
					<td>需求名称：</td>
					<td><input type="text" className="demand-name" maxLength="12" required/><span className="info">为您的需求命名，限制 12 个字</span>
					<p className="error">请填写需求名称</p></td>
				</tr>
				<tr>
					<td style={{"vertical-align":"top","line-height":"30px"}}>需求描述：</td>
					<td><textarea maxLength="150" className="descriptionDemand" required></textarea><p><span className="length">还可以输入 <span className="size2">150</span> 个字</span></p>
					<p className="error">请填写需求描述</p></td>
				</tr>
				<tr>
					<td>产品名称：</td>
					<td>
						<input  type="text" maxLength="12" className="pro-name" name="proname" autoComplete="on" required/><span className="info">您的产品或服务名，限制 12 个字</span>
				...
				<tr>
					...
					<td>
						<textarea maxLength="150" className="introductionPro"  placeholder="产品或服务介绍，可上传附件，可填入网站链接" required></textarea>
						<p>是否上传产品介绍附件：<label><input type="radio" name="isFJ" required/>是</label><label><input type="radio" name="isFJ" defaultChecked required/>否</label><span className="length">还可以输入 <span className="size">150</span> 个字</span></p>
				...
				<tr>
					...
					<td className="date-p"><input autoComplete="off" type="text" className="form_date startDate" readonly /><i className="date_icon"></i><span>结束日期:</span><input autoComplete="off" type="text" className="form_date endDate" readonly /><i className="date_icon"></i>
					<p className="error"></p></td>
				</tr>
				<tr>
					...
					<td>
						...
						<p className="radio_fz"><span>负责人姓名：</span><input type="text" style={{"width":"120px"}} className="pleader"/><span style={{"marginLeft":"50px"}}>负责人电话：</span><input autoComplete="off" type="text" style={{"width":"162px"}} className="pphone"/></p>
...
class AreaAndRequirement extends React.Component{
				...
				return this.state.chooseCity.map(function (city,index){
					for(var key in city){
						if(key === data[0] || key === data[1] || key === data[2]){
							return (<li title={city[key].regionName}  className="click js_key" href="javascript:;" ><a>{city[key].regionName}</a></li>);
						}
					}
				}.bind(this));
			}else{
				return this.state.chooseCity.map(function (city,index){
					for(var key in city){
						if(key === data[0] || key === data[1] || key === data[2] || key === data[3]){
							return (<li data-name={city[key].regionPy} title={city[key].regionName}  className="click js_key" href="javascript:;" ><a>{city[key].regionName}</a></li>);
	...
	areaListList(){
		...
		return this.state.countNumber.map(function (name,index){
			return (
					<div data-id={index} className="areaList">
					...
					<div className="input">
						<label>发布数量:</label>
						<input type="text" required="" className="only number size" title="" />
						<i>条</i>
						<p className="error">请填写发布数量</p>
					</div>
					<div className="input">
						<label>单价:</label>
						<input type="text" required="" className="price number size" title="" />
						<i>元</i>
	...
	render() {
		return (
          ...
          <tr>
              ...
              <td width="470px"><label><input type="radio" name="isXS" required/>是</label><label><input type="radio" name="isXS" defaultChecked required/>否</label>
          ...
          <tr className="radio_xs">
              ...
              <td><div className="ossfile2" style={{"width":"370px"}}></div></td>
              <td><button type="button" className="btn" id="postfiles2" style={{"width":"152px"}}>开始上传</button></td>
          </tr>
          <tr>
              <td>是否上传话术：</td>
              <td><label><input type="radio" name="isHS" required/>是</label><label><input type="radio" name="isHS" defaultChecked required/>否</label></td>
          </tr>
          <tr className="radio_hs">
              ...
              <td><div className="ossfile3"  style={{"width":"370px"}}></div></td>
              <td><button type="button" className="btn" id="postfiles3"  style={{"width":"152px"}}>开始上传</button></td>
          </tr>
          <tr>
              <td>是否上传目标客户名单：</td>
              <td><label><input type="radio" name="isMD" required/>是</label><label><input type="radio" name="isMD" defaultChecked required/>否</label></td>
          </tr>
          <tr className="radio_md">
              ...
              <td><div className="ossfile4"  style={{"width":"370px"}}></div></td>
		...
		return (
			<div className="context-explain package package1">
             <div>
				 ...
				 <tr>
				 <td colSpan="4" className="countPrice"><span className="cash">￥</span><span className="price">0</span></td>
				 </tr>
				 <tr className="hide">
    ...
    render(){
         return (
                 <div>
                     ...
                     <div className="areaList_add"><button type="button"><i className="add"></i><span>添加区域</span></button><span>还可以添加<span className="length" > 4 </span>个区域</span></div>
                 </div>
         )
     }
    }
...
React.render(<AreaAndRequirement/>,$("#new_demand div.demend_left .form_col4>.main")[0]);
React.render(<Fj />,$("#new_demand div.demend_left .form_col5>.main")[0]);
React.render(<Preview/>,$("#new_demand div.demend_right .right").eq(0)[0]);
