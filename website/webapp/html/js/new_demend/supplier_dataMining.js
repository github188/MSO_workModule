...
var Yewu=React.createClass({
    ...
    render:function(){
        if(this.state.data==null){
            ...
            <tr>
            <td>筛选渠道:</td>
            <td><span>{isNull(this.state.data.cleaningchannel)?"暂无":this.state.data.cleaningchannel}</span></td>
            </tr>
            <tr>
            <td>竞拍数量:</td>
            ...
            <tr>
            <td className="hideSub">单&emsp;&emsp;价:</td>
            <td><span>{isNull(this.state.data.orderprice)?"暂无":this.state.data.orderprice+"元"}</span></td>
            </tr>
            <tr>
            <td>竞拍手续费:</td>
            <td><span>{isNull(this.state.data.biddingcommission)||this.state.data.biddingcommission==0||this.state.data.favorableway==1?"免手续费":this.state.data.biddingcommission+"元"}</span></td>
            </tr>
            <tr>
            <td>拨打次数:</td>
            <td><span>无人接听数据最多拨打三次</span></td>
...
var State=React.createClass({
    ...
    render:function(){
        ...
        <div className="title-explain"><span style={{"font-size":"16px"}}>订单进度</span></div>
            ...
            <tr>
            <td colSpan="2"><div className="progress"><b>{parseInt(fishnum/auctionnum*100)}%</b><div className="progress-bar brown" style={{"max-width":"100%","width":(auctionnum!=0?parseInt(fishnum/auctionnum*100):0)+"%"}}></div>
            <div className="bublle"><p>竞拍量：{auctionnum}</p><p>完成量：{fishnum}</p></div></div></td>
            </tr>
            <tr>
...
var Progress_speed=React.createClass({
    ...
    render:function(){
        ...
        <div className="bottom"><button type="button" className="button_close btn_tj">确&nbsp;定</button></div>
        </div>
        <div style={{"width":"900px"}} className="upload_info">
            <div className="top"><span>成单报告上传记录</span><i className="close1"></i></div>
            <div className="main" >
...
$.ajax({
    ...
    success:function(data){
        ...
        if(data.code=="Y"){
            $("div.title_select p").html(data.orderDetail.ordername);
