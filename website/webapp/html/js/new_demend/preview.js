	...
	updataStructrue(id){
		...
		this.getIndustryData(function (onlineAndOffline){
				...
				if(this.props.industry){
					...
					React.render(<Preview name='1' data={areaList()}/>,$(".demend_right .right")[0]);
				}
	...
	render() {
		return (
		  <table className="crowd">
			<tr>
                <td style={{"vertical-align": "text-top","line-height":"30px"}}>目标人群：</td>
                <td  style={{"vertical-align": "text-top","line-height":"30px"}}>
...
class SaleTemplate extends React.Component{
	...
	/*创建标签名称*/
		return this.state.freelabel.map(function (name,index){
					return (<li>{name.labelName}</li>);
		       });
	}
	createchargelabel(){
	/*创建标签名称*/
		return this.state.chargelabel.map(function (name,index){
					return (<li title={name.labelPrice}>{name.labelName}<i></i></li>);
		       });
	}
	addshow(){
			...
			<div>
				...
				<div className="retail-template">
					...
					<div className="add-label">
						<div className="label_list-title"><span>付费标签</span><span>已选择：&nbsp;&nbsp;{this.state.activelength}&nbsp;&nbsp;个</span><span>单价：&emsp;+<span className="price">{this.state.activeprice.toFixed(2)}</span>&nbsp;&nbsp;元</span><span className="add-show">点击展开<i></i></span></div>
					...
					<div className="label_add" style={{"font-size":"12px"}}>
						
						<span>添加自定义标签，已添加 {this.state.activelength} 个；
			...
			<table className="messsage-table">
					...
					<tr>
							...
							<td className="date-p">
								...
								<input autoComplete="off" placeholder="结束日期" type="text" className="form_date end_date" readOnly /><i className="date_icon"></i>
...
if(newDandData){
	React.render(<Yewu industry={JSON.parse(newDandData).industry} name="1"/>,React.rootEl.find(".form_col1>.main")[0]);
}else{
	React.render(<Yewu name="1"/>,React.rootEl.find(".form_col1>.main")[0]);
}

if(newDandData){
	React.render(<Mbkh name='1' taskPeople = {JSON.parse(newDandData).taskPeople} />,React.rootEl.find(".form_col2>.main")[0]);
}else{
	React.render(<Mbkh name='1' />,React.rootEl.find(".form_col2>.main")[0]);
}
...
if(newDandData){
	React.render(<AreaAndRequirement  city={JSON.parse(newDandData).city} name='1' />,React.rootEl.find(".form_col3>.main")[0]);
	
}else{
	React.render(<AreaAndRequirement name='1' />,React.rootEl.find(".form_col3>.main")[0]);

}


if(newDandData){
	React.render(<SaleTemplate saleTem = {JSON.parse(newDandData).saleTem } name='1' />,React.rootEl.find(".form_col4>.main").eq(1)[0]);
}else{
	React.render(<SaleTemplate name='1' />,React.rootEl.find(".form_col4>.main").eq(1)[0]);
}

/*描述*/
if(newDandData){
	React.render(<EssentialInformation discrible = {JSON.parse(newDandData).discrible } name='1' />,React.rootEl.find(".form_col5>.main")[0]);
}else{
	React.render(<EssentialInformation name='1' />,React.rootEl.find(".form_col5>.main")[0]);
}

React.render(<Preview name='1' data={areaList()}/>,$(".demend_right .right")[0]);

React.render(<Progress name='1' />,$(".demend_left .progress_speed")[0],function(){console.timeEnd("test");});
