...
class SaleTemplate extends React.Component{
	...
	resetStrcture(){
		...
		if(selectedlist[0] && add_label_list[0]){
			...
			this.rootEl.find('.label_list-title .price').html(countPrice+'.00');
	...
	render(){
		return (
			...
			<p className="color">请选择需要的标签，标签即成单信息字段</p>
				...
				<div className="diylabel_box" style = {{'display':'none'}}>
				</div>
...
resetStrcture(function (data){
	...
	React.render(<Yewu industry={data} name="1"/>,React.rootEl.find(".form_col1>.main")[0]);
	
	React.render(<Mbkh targethumen={data}  name='1' />,React.rootEl.find(".form_col2>.main")[0]);
	
	React.render(<AreaAndRequirement area={data} name='1' />,React.rootEl.find(".form_col3>.main")[0]);
	
	React.render(<SaleTemplate element={data}  name='1' />,React.rootEl.find(".form_col4>.main").eq(1)[0]);
	
	React.render(<EssentialInformation information = {data} name='1' />,React.rootEl.find(".form_col5>.main")[0]);
	
	React.render(<Preview name='1' data={areaList()}/>,$(".demend_right .right")[0]);
	React.render(<Progress name='1' />,$(".demend_left .progress_speed")[0]);
