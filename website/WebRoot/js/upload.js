...
//                { title : "Zip files", extensions : "zip,rar,docx,doc,xls,xlsx"}
//            ],
    ...
    init: {
        ...
        FilesAdded: function(up, files) {
            plupload.each(files, function(file) {
                ...
                $('.ossfile')[0].innerHTML = '<div id="' + file.id + '">' + file.name + ' (' + plupload.formatSize(file.size) + ')'
        ...
        FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                ...
                file1=f1;
                //上传成功后回调用  需求发布 上传附件页面
                if("customer_lj_DemandManagement2"==pagename){
    ...
    init: {
        ...
        FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                ...
                //上传成功后回调用   上传附件页面
                if("customer_lj_DemandManagement2"==pagename){
                	document.getElementById("xsxsurlcheckfile").style.display="";
                }
            }
            else
            {
    ...
    init: {
        ...
        FilesAdded: function(up, files) {
            plupload.each(files, function(file) {
                ...
                $('.ossfile4')[0].innerHTML = '<div id="' + file.id + '">' + file.name + ' (' + plupload.formatSize(file.size) + ')'
                    +'<div class="progress"><div class="progress-bar" style="width: 0%"><b></b></div></div>'
                    +'</div>';
            });
        },
...
uploader4.init();
//绑定文件添加进队列事件
uploader4.bind('FilesAdded',function(uploader4,files){
    for(var i = 0, len = files.length; i<len; i++){
        var file_name = files[i].name; //文件名
        //构造html来更新UI
        !function(i){
