...
class OrderList extends Component {
    ...
    componentDidMount() {
        $(".ant-select-tree").css({fontSize: "16px"})
        $(".ant-table-thead>tr>th").css({backgroundColor: "#EDEDEB", fontSize: "16px"})
        $(".ant-table-tbody").css({fontSize: "16px", color: "#333333"})
        this.getData();//获取全部数据
    }
    ...
    updateData(result = []) {
        ...
        for (let i = 0; i < data.length; i++) {
            var index = Object.assign({}, data[i]), newObj = {}
            ...
            if (index.orderState == "1") {
                index.operate =
                    (<div><Button onClick={this.getOrder.bind(this, index.id)}
                                  style={{backgroundColor: "#0098D8", float: "left", color: "#ffffff"}}>立即领取</Button>
                        <Button
                            onClick={this.detail.bind(this, index.id, index.createTime, index.orderState)}
                            icon="edit"
                            style={{backgroundColor: "#8F44AD", color: "#ffffff", float: "right"}}>查看</Button></div>)
            } else {
            ...
            index.createTime = moment(index.createTime).format("YYYY-MM-DD");
            ...
            var obj = Object.assign({}, index, newObj, {key: i})
    ...
    updateClose(num) {
        ...
        if (num == 1) {
            //TODO 更改状态 orderState  3,4  放弃取消  5 进行中
            //根据订单号替换此条数据
            this.updateState(this.state.updateOrderId, 5)
        }
     ...
     * @param orderId  订单id
     * @param state    2:领取3:放弃
     */
    updateState(orderId, state) {
        var _this = this;
        $.ajax({
            url: `${domain}/worker/receiveorder/${orderId}/${state}`,
            type: "patch",
            contentType: "application/json",
            success: function (result) {
                if (result.msg == "success") {
    ...
    render() {
        return (<div style={{display: this.props.show ? "block" : "none", padding: "40px 20px"}} className="root">
            ...
            <div className="select" style={{paddingBottom: "20px"}}>
                <span style={{paddingRight: "7px"}}>订单状态:</span>
                <TreeSelect
                    style={{width: 226, fontSize: 16}}
                    value={this.state.value}
                    dropdownStyle={{maxHeight: 400, overflow: 'auto', fontSize: 16}}
                    treeData={treeData}
...
class Detail extends Component {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.msg == "success") {
                    //TODO  重置数据
                    result.data.orderState = _this.transformState(result.data.orderState);
                    _this.setState(Object.assign({}, _this.state, result.data, {state: "", hour: "00", minute: "00"}))
                }
    ...
    render() {
        return (<div style={{padding: "20px", display: !this.props.prop.hide ? "block" : "none"}}>
            <Pop close={this.state.close} cb={this.updateClose.bind(this)}
                 content={this.state.state == 5 ? "确认领取此订单吗？" : "确认放弃此订单吗？"}/>
            <div className="cont-box">
                <div className="back" onClick={() => {
                    this.props.back(true, "")
                }}><img style={{display: "inline-block"}} src={back}
                        alt=""/>返回
                </div>
                ...
                <div className="details-box">
                    <ul>
                        ...
                        <li>
                            <label>业务类型：</label>
                            <span> {this.state.serviceType == "1" ? "销售线索挖掘" : (this.state.serviceType == "2" ? "数据加工" : "标签匹配")}<span
                                className="bubble-box">
                                <Tip title={"对用户行为和特征进行分析，通过有效触达，挖掘意向客户"} pos={"bottomLeft"}/>
                                </span>
                            </span>
                        </li>
                        <li>
                        ...
                        <li style={{display: this.state.serviceType == "1" ? "block" : "none"}}>
                            <label>目标区域：</label>
                        ...
                        <li>
                            <label>结算时间：</label>
                            <span>{this.state.settlementCycle}</span>
                        </li>
                        ...
                        <li className="invoice ">
                            <label>发票类型：</label>
                            <span>{this.state.invoiceType == "0" ? "不使用" : "MSO平台，按结算金额8%收费"}</span>
                        </li>
                        <li>
                            <label>订单服务费：</label>
                        ...
                        <li>
                            <label>质检标准附件：</label>
                            <span><a href={this.state.quality}>下载</a></span>
                        </li>
                        <li>
                            <label>精准信息服务：</label>
                        ...
                        <li>
                            <label>销售线索模板：</label>
                            <span><a href={this.state.sales}>下载</a></span>
                        </li>
                        <li>
                            <label>订单状态：</label>
                        ...
                        <li>
                            <label>话术文件：</label>
                            <span><a href={this.state.speechcraft}>下载</a></span>
                        </li>
                    </ul>
                    {/*--待领取-*/}
                    <div className="order-state" style={{display: this.state.orderState == "待领取" ? "block" : "none"}}>
                        <p>此订单剩余领取时间还有：
                            <span className="red"
                                  style={{paddingLeft: "28px"}}>{this.state.hour + "小时" + this.state.minute + "分"}
                                <Tip title={"每个订单的领取时间为24小时，超过时间未领取的订单将被视为自动放弃"} pos={"rightTop"}/>
                        </span>
                        </p>
                        <div className="btn-box">
                            <Button type={"primary"} style={{marginRight: "20px"}} disabled={!this.state.checked}
                                    onClick={this.getOrder.bind(this, 5)}>立即领取</Button>
                            <Button onClick={this.getOrder.bind(this, 3)}>放弃订单</Button>
                        </div>
                        <label className="ant-checkbox-wrapper">
                            <Checkbox onChange={this.select.bind(this)}>
                    ...
                    <div className="order-state"
                         style={{display: this.state.orderState == "已放弃" || this.state.orderState == "已取消" ? "block" : "none"}}>
    ...
    render() {
        return (<div>
            {this.state.listShow ? <OrderList detail={this.cb.bind(this)} show={this.state.listShow}/> :
                <Detail prop={{...this.state}} back={this.cb.bind(this)}/>}
        </div>)
    }
