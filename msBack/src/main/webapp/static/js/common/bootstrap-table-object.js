...
(function () {
    ...
    var MSOTable = function (bstableId, url, columns) {
        this.btInstance = null;					//jquery和BootStrapTable绑定的对象
        this.bstableId = bstableId;
        this.url = url;
        this.method = "get";
    ...
    MSOTable.prototype = {
            ...
            this.btInstance =
                $('#' + tableId).bootstrapTable({
                  ...
                  // responseHandler: function(data){
                   ...
                   //},
                    responseHandler: this.responseHandler_define,
                    toolbar: "#" + this.toolbarId,//顶部工具条
                    striped: true,     			//是否显示行间隔色
                    cache: false,      			//是否使用缓存,默认为true
                    pagination: true,     		//是否显示分页（*）
        ...
        setResponseHandler: function (handler) {
           // handler();
            ...
            this.responseHandler_define = handler;
    ...
    var msobackTable = function (options) {
        this.btInstance = null;					//jquery和BootStrapTable绑定的对象
		this.toolbarId = options.bstableId + "Toolbar";
        return $.extend(true,this, options);
    };
    
    msobackTable.prototype = {
        /**
         * 初始化bootstrap table
         */
        ...
        init: function () {
            ...
            this.btInstance =
                $('#' + _this.bstableId).bootstrapTable({
                    contentType: "application/x-www-form-urlencoded",
                    url: this.url,				//请求地址
                    method: _this.method,		//ajax方式,post还是get
                    ajaxOptions: {				//ajax请求的附带参数
                        data: _this.data
                    },
					queryParams: function (param) {
						return _this.queryParams(param);
                    }, 
                    responseHandler:function (param){
						return _this.responseHandler(param);
					},
					onLoadSuccess:_this.onLoadSuccess,
                    toolbar: "#" + _this.toolbarId,//顶部工具条
                    striped: true,     			//是否显示行间隔色
                    cache: false,      			//是否使用缓存,默认为true
                    pagination: true,     		//是否显示分页（*）
                    sortable: true,      		//是否启用排序
                    ...
                    pageList: [10, 50, 100],  	//可供选择的每页的行数（*）
                    queryParamsType: 'limit', 	//默认值为 'limit' ,在默认情况下 传给服务端的参数为：offset,limit,sort
                    sidePagination: _this.paginationType,   //分页方式：client客户端分页，server服务端分页（*）
                    search: true,      		//是否显示表格搜索，此搜索是客户端搜索，不会进服务端
                    strictSearch: false,			//设置为 true启用 全匹配搜索，否则为模糊搜索
                    showColumns: true,     		//是否显示所有的列
                    showRefresh: true,     		//是否显示刷新按钮
    ...
    window.msobackTable = msobackTable;
